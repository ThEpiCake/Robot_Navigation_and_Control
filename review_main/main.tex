...
עבור פרויקט זה תוכנן ומונג מניפולטור רובוטי בעל 6 דרגות חופש (\textenglish{DOF}) בתצורת \textenglish{R-P-P-R-R-R} מותאמת אישית (\texttt{custom\_rpp\_zyz\_robot}). הרובוט תוכנן לבצע משימות איסוף והנחה (\textenglish{Pick and Place}) במרחבים צפופים כגון מדפי ספרים.
...
$^2A_3$ & גובה האסיפה הנוטה (ציר הכיפוף) & 0.38 \\
...
\begin{itemize}
    ...
    \item $G_2 = 89.271\,\text{N}$: המפרק הפריזמטי הראשון ($d_1$) מרים את כל המסה שמעליו ישירות נגד הכובד. מאחר שה-URDF מגדיר את arm1\_outer\_link כשרוול חיצוני קבוע (לא נע עם $d_1$), הנע הפנימי ומעלה (arm1\_inner + שני צמד הטיות + שתי זרועות arm2 + 4 חוליות כף יד + wrist\_z2 + גריפר ואצבעות) שוקל בדיוק $9.1\,\text{kg}$: \[ 9.1\,\text{kg} \times 9.81\,\text{m/s}^2 = 89.271\,\text{N} \]
    \item $G_6 = 0$: מפרק $\theta_6$ הוא הסיבוב האחרון של כף היד סביב ציר האיבר (ציר הגריפר). הגריפר סימטרי סביב ציר זה, ולכן מרכז המסה שלו נמצא על הציר ואינו עולה בגובה עם שינוי $\theta_6$ — בדומה לסיבוב ברגי על ציר עצמו.
\end{itemize}
...
\end{equation}
כאשר $J_{v,k},\,J_{\omega,k}$ הם יאקוביאני מהירות ליניארית וזוויתית של חוליה $k$, ו-$I_k^{\text{world}}=R_k I_k^{\text{body}} R_k^T$. הגזירה בוצעה ב-\textenglish{SymPy} עם אריתמטיקת \textenglish{Rational} מדויקת, ולכן הביטויים הסימבוליים נקיים לחלוטין (ללא שגיאות \textenglish{floating-point}). המטריצה היא סימטרית וחיובית לחלוטין בכל תצורה. להלן הכניסות השונות מאפס של המשולש העליון, מקובצות לפי משמעות פיזיקלית.
...
\end{equation}
הגורם $\tfrac{51}{20}d_2^2$ הוא תרומת משפט שטיינר (\textenglish{Parallel Axis Theorem}): הארכת $d_2$ מרחיקה את מרכזי המסה של חוליות כף היד מציר $\theta_1$, ומגדילה את מומנט ההתמדה בפרופורציה ריבועית. הפונקציה $g(\theta_4,\theta_5,\theta_6)$ מכילה תרומות כף היד (פרטים ב-\texttt{derivation.md}).

\paragraph{בלוק כף היד ($\theta_4,\theta_5,\theta_6$) — ללא תלות ב-$\theta_1$.}
לאחר יישום \textenglish{trigsimp}, אף כניסת כף-יד אינה תלויה ב-$\theta_1$, כפי שמצופה מסימטריה סיבובית סביב ציר הבסיס:
\begin{align}
...
\end{align}
הערכים הקטנים (סדר $10^{-4}$ עד $10^{-2}$) משקפים את אינרציית חוליות כף היד הקטנות (מסות 0.5 kg, רדיוס ס"מ בודד).
...
\end{align}
כל שאר הכניסות ($M_{12},M_{14},M_{15},M_{26},M_{34},M_{36},M_{56}$ ומקביליהן) הן אפס מבני. הכניסות $M_{14}$ ו-$M_{15}$ אינן אפס אך מורכבות יותר (פרטים ב-\texttt{derivation.md}).
...
\end{equation}
$C$ מכילה 26 כניסות שונות מאפס מתוך 36, אך הביטויים הסימבוליים גדולים במיוחד (כ-$5.9$ MB בסך הכל) ואינם מתאימים להדפסה. $C$ נשמרת כפונקציה \textenglish{NumPy} מהירה דרך \textenglish{lambdify+CSE}.

\subsection{תהליך הפיתוח הסימבולי בקוד (\textenglish{Software Implementation})}
חישוב וגזירת המטריצות מבוצע באמצעות מחולל קוד סימבולי בפייתון:
\begin{itemize}
...
וקטור המצב $x \in \mathbb{R}^{12}$ מורכב ממיקומי ומהירויות המפרקים. נגזרתו מחושבת על ידי:
\begin{equation}
...
\end{equation}
\textbf{הערת מימוש:} חישוב $\ddot{q}$ דורש פתרון המערכת $M(q)\ddot{q} = \tau - C\dot{q} - G$. במקום לחשב את $M^{-1}$ במפורש — פעולה שמחייבת $O(n^3)$ פעולות ופגיעה בדיוק נומרי כאשר $M$ קרובה לסינגולרית — הקוד (\texttt{eom\_rhs}) משתמש בפותרן הליניארי \texttt{np.linalg.solve(M, rhs)} המבוסס על פירוק LU. שיטה זו יציבה יותר ומהירה פי $\sim3$ עבור מטריצה $6\times6$.

\subsection{שיטת האינטגרציה: \textenglish{Runge-Kutta} מסדר 4 (\textenglish{RK4})}
האינטגרציה מתבצעת בפסיעת זמן $dt = 0.001$:
\begin{align}
k_1 &= f(t_n, x_n, \tau_n) \nonumber \\
...
\begin{enumerate}
    ...
    \end{equation}
    כל סחיפה בגרף $E_{total}(t)$ נובעת אך ורק משגיאת הגזירה הנומרית של \textenglish{RK4}: שגיאה מקומית מסדר $O(dt^5)$ ושגיאה גלובלית מסדר $O(dt^4)$ — לפסיעת זמן $dt=0.001$ שניות, הסחיפה הנומרית צפויה להיות $\sim 10^{-6}$ ג'אול בלבד לאורך עשרות שניות. ערך זה אמת מידה ישירה לנכונות $M(q)$, $C(q,\dot{q})$ ו-$G(q)$.
    \item \textbf{תגובה חופשית פיזיקלית}: הרובוט נע תחת כוח הכובד בלבד מהמיקום ההתחלתי, ומפרקים פריזמטיים שוקעים ומפרקים סובבים תנודים — כפי שצופה מהתיאוריה.
\end{enumerate}
...
\begin{figure}[h]
    ...
    \caption{גרפי פרמטרי הקונפיגורציה כפונקציה של זמן במצב תגובה חופשית ($\tau=0$). ניתן לראות את תנועת הרובוט תחת כוח הכובד בלבד — מפרקים פריזמטיים ($d_1, d_2$) שוקעים בהדרגה, ומפרקים סובבים ($\theta_1, \theta_4, \theta_5, \theta_6$) תנודים סביב שיווי המשקל.}
...
\end{equation}
מכאן שהבקר "מבטל" את $M$, $C$ ו-$G$ בצד שמאל ומשאיר רק מערכת לינארית שאת התגובה שלה ניתן לעצב ישירות על ידי בחירת $K_p, K_d, K_i$ — זהו היתרון המרכזי של שיטת \textenglish{CTC} על פני \textenglish{PID} רגיל. כלומר, חוק \textenglish{CTC} מבצע ביטול משוב מדויק של כל האינרציות הלא-לינאריות (כולל אפקטי קוריוליס, תרומות צנטריפוגליות ועומסי כובד), ומייצר דינמיקת שגיאה לינארית ויציבה לחלוטין: הפולינום האופייני $s^2 + K_d s + K_p = 0$ מקנה ריסון קריטי ($\zeta = 1$) ומבטיח $e(t) \to 0$ אקספוננציאלית עבור כל מפרק — ובלתי תלוי בתצורת הרובוט.

\subsection{כיול הבקר ומניעת רוויה}
לשיכוך קריטי ($\zeta=1$), הגברי הבקר הוגדרו לפי $K_p = \omega_n^2$ ו-$K_d = 2\zeta\omega_n = 2\omega_n$, כאשר תדרי הרזוננס לכל מפרק הם $\omega_n = [5, 4, 4, 6, 6, 6]$ ראד/שנייה. ההגברים הנבחרים הם:
\begin{align*}
K_p &= [25.0,\ 16.0,\ 16.0,\ 36.0,\ 36.0,\ 36.0] \\
K_d &= [10.0,\ 8.0,\ 8.0,\ 12.0,\ 12.0,\ 12.0] \\
K_i &= [0.5,\ 0.5,\ 0.5,\ 0.5,\ 0.5,\ 0.5]
\end{align*}
הגבר $K_i$ קטן מסלק שגיאות מצב מתמיד הנובעות מחוסר התאמה מושלם של המודל. בנוסף, הוגדר מנגנון \textenglish{Anti-Windup} המגביל את מצבר האינטגרטור לתחום $\pm 5.0$ [rad/m] בכל מפרק, למניעת הצטברות שגיאה ושמירה על יציבות.

\subsection{תוצאות תנועה בחוג סגור}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{../results/joint_states_pid.png}
    \caption{עקיבת קונפיגורציית המפרקים בזמן.}
...
\begin{figure}[h]
    ...
    \caption{שגיאת המעקב ומאמצי המנועים.}
...
\begin{figure}[h]
    ...
    \caption{חתכי זמן עוקבים של ייצוג ה-\textenglish{Stick-Figure} במהלך תנועת הרובוט בשליטת בקר ה-\textenglish{CTC+PID}. ניתן לראות את הרצף החלק של התנועה מנקודה A לנקודה B.}
...
\begin{figure}[h]
    ...
    \caption{מסלול קצה-האיבר (\textenglish{End-Effector}) במרחב המשימה בשליטת בקר ה-\textenglish{CTC+PID}. הנתיב המרחבי עובר מנקודת ההתחלה A $(0.455, -0.1, 0.15)$ לנקודת היעד B $(0.50, -0.30, 0.32)$ תוך הימנעות מהתנגשות עם חציצת המדף.}
    \label{fig:ee_path_pid}
\end{figure}


\section{תכנון תנועה חופשי מהתנגשויות (\textenglish{Motion Planning \& Obstacle Avoidance})}
\subsection{מסלול קינטי (\textenglish{Quintic Polynomial Trajectory})}
לאחר שנקבעות נקודות הדרך ($q_0, q_f$) לכל קטע, נוצר מסלול רציף בעל תאוצה אפסית בקצות (מינימום-ג'רק) באמצעות פולינום מסדר 5:
\begin{equation}
...
מסלול זה מספק גם $\dot q_d$ וגם $\ddot q_d$ לבקר ה-\textenglish{CTC} ומבטיח השקה ונחיתה חלקות ללא בלמה פתאומית.

\subsection{מידול גיאומטרי ותכנון גלובלי}

\textbf{מדוע לתכנן ב-\textenglish{C-Space}?} \\
מרחב הקונפיגורציה (\textenglish{C-Space}) הוא מרחב 6-מימדי שציר בכל ממד מייצג מפרק אחד. כל נקודה בו מתארת קונפיגורציה מלאה של הרובוט. תכנון ישיר בו עדיף על תכנון במרחב המשימה כי בדיקת התנגשות מתבצעת פעם אחת לכל קונפיגורציה, ומשפחות של קונפיגורציות בטוחות ($Q_{free}$) מוגדרות מראש. יתרון נוסף: קיומם של פתרונות IK מרובים אינו מסבך את התכנון.

\textbf{מודל הקפסולות:} כל חוליה מיוצגת כגליל (קפסולה) ברדיוס $r = 35\,\text{mm}$. בדיקת התנגשות מחשבת את המרחק המינימלי מקטע הציר של הגליל לכל AABB של מכשול. קונפיגורציה חפשייה מוגדרת: $\text{clearance}(q) > r + \text{margin}$, כאשר $\text{margin} = 10\,\text{mm}$ (סף סה"כ: 45 מ"מ).
...
\begin{enumerate}
    \item \textenglish{EXTEND}($\mathcal{T}_A$, $q_{rand}$): קח דגימה אקראית (או את $q_{goal}$ בהסתברות של 20\%), מצא את הצומת הקרוב בעץ A, הרחב לעברו בצעד מנורמל.
    \item \textenglish{CONNECT}($\mathcal{T}_B$, $q_{new}$): נסה לחבר את עץ B אל הצומת החדש בעץ A — חזור על EXTEND לכיוון $q_{new}$ עד להגעה או חסימה. כך גם עץ B גדל בכל ניסיון חיבור.
    \item עם חיבור מוצלח — שרשר את נתיב עץ A (התחלה$\rightarrow$פגישה) עם נתיב עץ B הפוך (פגישה$\rightarrow$יעד), ומקצר על ידי \textenglish{shortcutting} אקראי (400 ניסיונות).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \texttt{\textenglish{controllers.py}}: בקר \textenglish{CTC+PID} עם \textenglish{Anti-Windup}, מחולל מסלול קינטי (\textenglish{quintic polynomial}), ובסיסי השוואה: \textenglish{feedforward} בלבד ו-\textenglish{PID} טהור.
    ...
    \item \texttt{\textenglish{gazebo\_control\_node.py}}: צומת \textenglish{ROS2} המשמיע את המסלול מה-\textenglish{CSV} לבקר ה-\textenglish{Gazebo} בזמן אמת.
\end{itemize}

\subsection{אינטגרציה עם \textenglish{ROS2} ו-\textenglish{Gazebo}}
המסלול המחושב ב-\textenglish{Python} מיוצא לקובץ \texttt{pid\_trajectory.csv}.
צומת \texttt{gazebo\_control\_node.py} קורא את הקובץ ומפרסם פקודות מיקום לכל אחד מ-6 המפרקים בתדר גבוה
לבקר \texttt{arm\_position\_controller} (מסוג \textenglish{ForwardCommandController}) שבתוך \textenglish{Gazebo}.
בכך, \textenglish{Gazebo} משמש כפלטפורמה חזותית ולאימות ה-\textenglish{URDF} ביחס לסימולציה הדינמית שנעשתה ב-\textenglish{Python}.

\subsection{מסקנות}
הפרויקט מדגים בהצלחה תכנון, מודל מתמטי, ובקרת \textenglish{CTC} למניפולטור בן 6 דרגות חופש. להלן עיקרי הממצאים:

\begin{itemize}
    \item \textbf{אוילר-לגראנז':} גזירה סימבולית אוטומטית ב-\textenglish{SymPy} עם \textenglish{CSE} הצליחה לייצר קוד נומרי רץ בזמן אמת, דבר שלא היה מעשי בגישה ידנית עבור שרשרת קינמטית מורכבת כזו.

    \item \textbf{CTC כ"ביטול דינמיקה":} הוכח שחוק הבקרה $\tau = M v + C\dot{q} + G$ הופך את הדינמיקה הלא-ליניארית של המניפולטור למערכת לינארית מפוצלת, מה שמאפשר כיול ישיר של הגברים לפי תדר הרזוננס הרצוי לכל מפרק.

    \item \textbf{שכבות בקרה:} שילוב של שתי שכבות — תכנון גלובלי (\textenglish{biRRT}) ומסלול חלק (קינטי) — הוכח כמספיק לסביבת המדף הנוכחית. התכנון הגלובלי מבטיח מסלול חף מהתנגשויות, ומסלול הקינטי מבטיח שהבקר מקבל $\dot{q}_d$ ו-$\ddot{q}_d$ חלקים שמונעים בלמות פתאומיות.

    \item \textbf{אתגר ההיצמדות (Narrow Passage):} מעבר הזרוע בין שני מדפים הצריך כוונון של מרווח הבטיחות המינימלי: ערך גדול מדי מונע ממצבי התחלה/יעד חוקיים להיחשב כ-$Q_{free}$, ערך קטן מדי מאפשר שגיאות מעקב שמובילות לגע בפועל.
\end{itemize}
% ==================== סוף חלק 1 ====================
\newpage
...
חלק זה מציג שיטת ניווט לחקר סביבה אוטונומי, מבוסס על המאמר
...
השיטה משמשת כבסיס לתוכנת ניווט הרחפן שפותחה במסגרת פרויקט גמר העוסק בחיפוש שטחי עם נחיל רחפנים אוטונומיים ב-\textenglish{ROS2/Gazebo}, בסביבה ממופה-אפס (ללא \textenglish{GNSS}).

\section{המאמר הנבחר — \textenglish{FUEL}}

\subsection{רקע ובעיה}

חקר אוטונומי (\textenglish{Autonomous Exploration}) הוא אחת המשימות הבסיסיות ביותר ברובוטיקה: כיצד ינווט רחפן לא-מאוייש (\textenglish{UAV}) בסביבה לא-ידועה, תוך כיסוי מירבי של השטח תוך זמן מינימלי ובבטחה? שיטות קלאסיות מבוססות \textenglish{frontier} (גבול החלל הנחקר) סובלות משני מחסומים:
\begin{itemize}
    \item חישוב מחדש של כל ה-\textenglish{frontiers} בכל שלב — עלות $O(N)$ ב-N תאי המפה
    \item תכנון גלובלי ממועט — הרחפן בוחר \textenglish{frontier} מקומי בלבד, ללא ראייה כלל-מפה
\end{itemize}
...
\begin{enumerate}
    \item \textbf{\textenglish{Frontier Information Structure (FIS)}}: מבנה נתונים מוגדל בצורה מצטברת (\textenglish{incremental}) המתעדכן אך ורק באזורים שנסרקו מחדש — לא סריקה מחדש של כל המפה. מאגר \textenglish{FIS} שומר לכל אשכול גבול: מיקום ממוצע, \textenglish{AABB}, נקודות תצפית אפשריות ועלויות קישוריות.

    \item \textbf{מתכנן גלובלי — סיור \textenglish{ATSP}}: ניתן את כל אשכולות ה-\textenglish{FIS} כעצירות, בעיית ה-\textenglish{Asymmetric TSP} מחושבת כדי לתכנן מסלול ביקור גלובלי יעיל. כך הרחפן מכסה את כל אזורי הגבול ברצף אופטימלי, לא רק את הקרוב ביותר.

    \item \textbf{כיוון מקומי + מסלול \textenglish{B-spline}}: ממחשב ה-\textenglish{ATSP} נבחר האשכול הבא. עבורו מתבצעת חיפוש בגרף לנקודת תצפית מועדפת, ולבסוף מחושב מסלול \textenglish{B-spline} מינימום-זמן הדינמי אפשרי ובטוח.
\end{enumerate}
...
\begin{itemize}
    ...
    \item תכנון \textenglish{ATSP} גלובלי — מונע "סחרחורת מקומית" בין גבולות סמוכים
...
\textenglish{swarm\_perception} & \textenglish{voxel\_mapper}: מיפוי \textenglish{log-odds} תלת-מימדי, גילוי גבולות ותכנון \textenglish{A*} \\
...
\textenglish{swarm\_msgs} & הגדרות מסרים מותאמות-אישית \\
...
\begin{enumerate}
    ...
    \item \textbf{\textenglish{slam\_to\_px4}} (\textenglish{swarm\_bringup, Python}): שיפוי שער רב-שלבי — בודק קפיצות מרחביות ($>$0.75 מ'), סחיפת \textenglish{yaw} ($>$0.55 ראד) וקצב יו SLAM ($>$90$^{\circ}$/ש'). עם שער פעיל: \textenglish{PX4 EKF2} חוזר ל-\textenglish{IMU} בלבד (עד 750 מ"ש — drift $<$0.2$^{\circ}$). היסטרזיס: פעיל אחרי 2 פריימים רעים, מתנקה אחרי 15 טובים.
...
שיטת הניווט בנויה משלוש שכבות מקוננות, המבוססות על ארכיטקטורת ה-\textenglish{FUEL} תוך הסתגלות הנדסית מכוונת לאילוצי הרחפן:
...
הרחפן מנוהל במצב \textenglish{Offboard} של \textenglish{PX4} ב\textbf{מצב הולונומי} (\textenglish{Holonomic Mode}): \textenglish{TrajectorySetpoint} שולח מהירות ישירות בשני הצירים $v_N, v_E$ ללא תלות בכיוון האף, כאשר \textenglish{yaw} נשלט בנפרד:

\begin{equation}
v_N = v_{max}\cos\psi_{VFH}, \quad v_E = v_{max}\sin\psi_{VFH}, \quad \dot{\psi}_{cmd} = \text{slew}(\psi_{target} - \psi_{current},\; \dot{\psi}_{max})
\end{equation}

\textbf{מדוע הולונומי ולא אוניציקל?} \\
הגרסה הקודמת של המערכת הפעילה \textenglish{Yaw-First Alignment}: הרחפן סובב לכיוון ה-VFH לפני כל תנועה. פרקטיקה זו גרמה לסחיפת SLAM חמורה — הסיבובים המהירים הכניסו רעש לקריאות החיישן ופגמו ביכולת ה-SLAM לעקוב אחר המיקום. \textbf{שינוי הולונומי פתר זאת}:
\begin{enumerate}
    \item כיוון ה-\textenglish{yaw} נשמר כמעט קבוע לכיוון הגפרון הנוכחי — ללא סיבובים מהירים.
    \item תנועה מבוצעת לכיוון שנבחר על ידי \textenglish{VFH} ישירות, גם אם שונה מכיוון האף.
    \item \textbf{עצירת רכיב קדמי}: אם \texttt{forward\_range < 0.45m} וזווית האף לכיוון \textenglish{VFH} $> 0.30$ ראד — $v_N = v_E = 0$ (עצירת בטיחות).
\end{enumerate}
...
הסתגלות זו ביחס ל-\textenglish{FUEL} המקורי (שמניח \textenglish{holonomic agent}) היא \textbf{ישרה יותר} מהגרסה האוניציקלית — ה-\textenglish{PX4 X500} מסוגל לתנועה הולונומית מלאה.

\subsection{מיפוי תלת-מימדי — \textenglish{voxel\_mapper}}
...
ערך גבוה ($0.85$) מסמן תא כ"תפוס" אחרי \textbf{פגיעה אחת}. במצלמת עומק עם רעש הגיאומטרי, דפנות ושטחים מוארים בחדות מייצרים פגיעות בודדות שמהוות רעש, לא מבנה. $l_{hit}=0.60$ דורש \textbf{שתי פגיעות} לפני סימון כ"תפוס" — תוצאה: מפה נקייה יותר, פחות "מריחה" בפינות.
...
\end{equation}
כאשר $G$ הוא רווח המידע (תאים לא-ידועים בשדה הראייה), $n_{cl}$ גודל האשכול, $d_{clr}$ מרווח מהמכשול הקרוב, $d_{path}$ אורך מסלול ה-\textenglish{A*}, $p$ פרוגרס אל מחוץ לאזור הידוע, ו-$y$ התאמת כיוון.

\subsection{תכנון נתיב גלובלי — \textenglish{A*} על מפת מעבר}

תהליך תכנון הנתיב (ב-2 \textenglish{Hz}):
\begin{enumerate}
    \item \textbf{מפת מעבר (\textenglish{passable map})}: תא עובר אם אינו חסום במסכת המכשולים וכלול באחת מאזורי העולם המותרים (חדר A, מסדרון, חדר B).
    \item \textbf{הגעה (\textenglish{BFS reachability})}: \textenglish{BFS} ארבע-כיווני מהרחפן על מפת המעבר. בועה מאולצת של 5 תאים + מסדרון \textenglish{Bresenham} לבית מבטיחים קישוריות מינימלית.
    ...
    \item \textbf{הוצאה}: מנתיב ה-\textenglish{A*} הגולמי נלקחות $\sim 8$ נקודות-ביניים (\textenglish{waypoints}) שמפורסמות כ-\textenglish{frontier\_path}.
\end{enumerate}

\textbf{הסתגלות הנדסית מכוונת — מ-\textenglish{ATSP} ל-\textenglish{A*} עם עונש \textenglish{Voronoi}}: \\
\textenglish{FUEL} המקורי מתכנן סיור \textenglish{ATSP} על כל אשכולות ה-\textenglish{FIS} ואז מייצר מסלול \textenglish{B-spline} מינימום-זמן. שתי הבחירות הבאות מהוות \textbf{הסתגלות הנדסית מכוונת}, לא מגבלה:
...
\begin{enumerate}
    ...
    \item \textbf{עיקרון הסחיבה/\textenglish{Retraction} בסביבה לא-ידועה}: בסביבות עם מדפים ועמודות צפופים, אופטימיזציית \textenglish{B-spline} עלולה להתכנס לנתיב העובר דרך תאים שעוד לא נסרקו (ידועים-כחופשיים לא-מאומת). \textenglish{A*} על \textenglish{passable map} — שכוללת רק תאים שנסרקו ואומתו כחופשיים — מממש באופן טבעי \textbf{עיקרון הסחיבה}: הרחפן נסוג לנתיב בטוח מאומת במקום לנסות לחצות אזורים לא-ידועים.
    \item \textbf{תקציב חישובי בזמן-אמת}: \textenglish{A*} עם עונש \textenglish{Voronoi} (מורכבות $O(V\log V)$) מסיים תכנון בפחות מ-500 מ"ש על מחשב-לוח — ומאפשר שכתוב מסלול בכל $T_{replan}=0.5$ שניות כאשר המפה מתעדכנת. \textenglish{ATSP} (מורכבות $O(n!)$) אינו ריאלי לחישוב בתדר זה.
\end{enumerate}

כפועל יוצא, שכבת ה-\textenglish{A*} מהווה מחולל ה\textenglish{Q\_free global route} הבטוח, המרוחק ממכשולים ומאומת כנגד מפת הוקסל העדכנית ביותר.
...
\textbf{בחירת כיוון}: מתוך כל "עמקי-פתיחה" נבחר מרכז העמק הקרוב לכיוון ה-\textenglish{waypoint}.

\textbf{פיקוד הולונומי} (ראה סעיף~\ref{sec:holonomic_model}):
\begin{equation}
    v_N = v_{max}\cos\psi_{VFH}, \quad v_E = v_{max}\sin\psi_{VFH}
\end{equation}
הרחפן נע לכיוון $\psi_{VFH}$ ישירות בלי לסובב תחילה את ראשו. הסרת ה-\textenglish{LiDAR} מחייבת שימוש בנתוני עומק ממצלמת \textenglish{OAK-D Lite} לבניית ה-\textenglish{VFH} — שדה ראייה מצומצם יותר (69.4$^{\circ}$ $\times$ 54.4$^{\circ}$) לעומת 360$^{\circ}$ של \textenglish{LiDAR}, אך מספיק לניווט בין מכשולים בתנאי הסימולציה.
...
\textbf{הערה}: \texttt{takeoff\_lookaround\_enabled=False} — סריקת 360$^{\circ}$ בהמראה הושבתה. היא הייתה גורם הסחיפה העיקרי של \textenglish{SLAM}: הסיבוב המהיר בתחילת המשימה גרם להתמרחות מיידית של המפה. ביטולה הביא לייצוב מלא.

מעבר שכבה ב-\textenglish{SCAN}: כאשר הכיסוי הכולל $\geq 82\%$ (\texttt{scan\_coverage\_fraction=0.82}) או אין \textenglish{frontiers} זמינים, הרחפן עולה 1 מ' ומתחיל שכבה חדשה. בסך-הכל 8 שכבות בגובה 1, 2, \ldots, 8 מ' מעל הקרקע.
...
מרחב המשימה של הרחפן הוא המרחב התלת-מימדי שבו מוגדרת תנועת מרכז הגוף. הוא מתואר על-ידי שלושת הקואורדינטות:
\begin{equation}
    \mathbf{p}(t) = (X(t),\; Y(t),\; Z(t)) \quad [\text{מ'}]
\end{equation}
כאשר $X$ = קואורדינטת \textenglish{North}, $Y$ = קואורדינטת \textenglish{East}, ו-$Z$ = גובה \textenglish{AGL} — כפי שנמדדים על-ידי \textenglish{slam\_pose\_fusion} (היתוך \textenglish{SLAM}+\textenglish{PX4}) ב-15 \textenglish{Hz}. הגרפים במרחב המשימה (מסלול \textenglish{XY} וגובה לאורך זמן) מוכיחים שהרחפן בצע את מסלול החקר הצפוי ועקב אחר פקודות \textenglish{frontier\_path}.
...
הריצה הסופית של המערכת (עם כל השינויים: VFH הולונומי, log\_odds=0.60, LiDAR מוסר, takeoff\_lookaround=False) הניבה:
...
תכיפות \textenglish{VFH} & 10 \textenglish{Hz} \\
תכיפות \textenglish{A*} & 2 \textenglish{Hz} \\
מינימה לוקאליים (לכידות) & 0 (\textenglish{VFH tangential escape}) \\
...
\begin{figure}[h]
    ...
    \caption{מסלול הרחפן בתלת-מימד (N, E, Alt). כל מצב ב-\textenglish{FSM} מוצג בצבע שונה. נצפות בבירור 8 שכבות הגובה כ"מדרגות" אופקיות. הרחפן חצה מחדר A דרך המסדרון לחדר B.}
...
\begin{figure}[h]
    ...
    \caption{ציר זמן מצבי מכונת המצבים (גאנט). שלושה פרקי \textenglish{TARGET\_INSPECT} מזוהים (איתור 3 עצמים). \textenglish{SCAN} מהווה את מרבית זמן המשימה ($\approx$1000 שניות).}
...
\begin{figure}[h]
    ...
    \caption{\textbf{שמאל:} צפיפות חקר במישור XY — כיסוי גבוה בחדר A, חלקי במסדרון, כניסה לחדר B. \textbf{ימין:} תפלגות שכבות הגובה — כל 8 שכבות נוכחות בנתוני \textenglish{SCAN}.}
...
\begin{figure}[h]
    ...
    \caption{למעלה: מספר סקטורי ה-\textenglish{VFH} החסומים (גולמי ולאחר הרחבה של $\pm 3$). למטה: אחוז הסקטורים החסומים. ערכים נמוכים לאורך מרבית המשימה — סביבה פתוחה; עליות ב-\textenglish{TARGET\_INSPECT} עם קירוב לעצם.}
...
הוראות התקנה והרצה מפורטות, כולל תיעוד מלא של כל פונקציה, נמצאות בקובץ ה-\texttt{\textenglish{README.md}} שבתיקיית הקורס בריפו.
...
עותק \textenglish{PDF} מלא של המאמר מצורף כנספח ב' בסוף מסמך זה, וזמין גם בריפו ה-\textenglish{GitHub}.

\subsection{קישורים לסרטוני הסימולציה}

שני סרטונים כלולים בתיקיית \texttt{\textenglish{results/}} שבריפו:
...
\begin{itemize}
    ...
    \item לוח בקרת המשימה: מפת ה-\textenglish{voxel} התלת-מימדית המתעדכנת בזמן-אמת + \textenglish{frontier\_path}
...
\begin{enumerate}
    ...
    \item \texttt{\textenglish{colcon build --symlink-install}} ואז \texttt{\textenglish{source install/setup.bash}} (מתיקיית השורש של הריפו)
...
\textenglish{SLAM} מתמרח & סיבוב 360$^{\circ}$ בהמראה & \texttt{takeoff\_lookaround\_enabled: False} \\
\textenglish{SLAM} מתמרח & שני חיישנים כותבים למפה & \texttt{integrate\_lidar\_into\_map: False} \\
\textenglish{SLAM} מתמרח & \textenglish{log-odds} גבוה מדי (רעש) & שינוי $l_{hit}$ מ-0.85 ל-0.60 \\
\textenglish{Yaw} oscillation & \textenglish{VFH} מחייב סיבוב לפני תנועה & \textenglish{VFH} הולונומי: \textenglish{yaw} מנותק \\
\textenglish{RTAB-Map} drift & \textenglish{motion blur} בסיבוב מהיר & זנח; \textenglish{PX4 EKF2} בלבד \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{שיפוי שער \textenglish{slam\_to\_px4}}: שכבת ההיסטרזיס (2 bad / 15 good) מגנה מסחיפת \textenglish{yaw} תוך שמירה על רציפות הניווט. כשהשער פעיל — \textenglish{EKF2} ממשיך ב-\textenglish{IMU} בלבד לפחות מ-750 מ"ש.

    \item \textbf{ניקוד אינטליגנטי}: משקלי מידע, מרווח, מרחק ופרוגרס בניקוד נקודות התצפית מכוונים את הרחפן לאזורים בעלי ערך חקר גבוה — בדומה לעקרון ה-\textenglish{viewpoint refinement} של \textenglish{FUEL}.
\end{itemize}

\subsection{הסתגלויות הנדסיות — בחירות מכוונות וטרייד-אופים}

הגישה שנבחרה כוללת מספר הסתגלויות מכוונות ביחס ל-\textenglish{FUEL} המקורי, כל אחת עם רציונל הנדסי ברור:

\begin{itemize}
    \item \textbf{בחירה: \textenglish{A*} במקום \textenglish{ATSP}}: אלגוריתם \textenglish{A*} עם עונש \textenglish{Voronoi} (ריכוז הנתיב במרכז המסדרון) הוכח ככלי גלובלי מספק לסביבות מחדרים וקוחים. \textenglish{ATSP} יתרום ביעילות מסלול גבוהה יותר בסביבות רב-חדריות עם עשרות אשכולות — אולם מצריך ספריית \textenglish{TSP} חיצונית (\textenglish{LKH / OR-Tools}) ותקציב חישובי רחב יותר. הרחבה זו מתוכננת לשלב הבא.

    \item \textbf{בחירה: \textenglish{VFH} הולונומי במקום \textenglish{B-spline}}: הרחפן \textenglish{X500} עם \textenglish{PX4 Offboard} יכול לבצע תנועה הולונומית מלאה — ה-\textenglish{VFH} ההולונומי מנצל זאת ישירות. \textenglish{VFH} מספק הימנעות ממכשולים בזמן-אמת (72 סקטורים $\times$ 10 \textenglish{Hz}) עם עלות חישובית זניחה. \textenglish{B-spline} מינימום-זמן יאפשר מהירויות גבוהות יותר (2--3 מ'/ש' לעומת $v_{max}=0.7$ מ'/ש' כיום), אך דורש שילוב \textenglish{MPC} לאכיפת הגבלות דינמיות — עבודה עתידית.

    \item \textbf{מגבלה טכנית — מסדרונות צרים}: ניפוח מכשולים של תא אחד (0.2 מ') עלול ל"נתק" \textenglish{BFS} מאשכולות גבול בסמוך לעמודים צרים. הפחתת \texttt{obstacle\_inflate\_cells} מ-2 ל-1 שיפרה זאת משמעותית. מגבלה זו ניתנת לתיקון מלא עם \textenglish{Bresenham corridor dilation} לאורך ציר המסדרון.

    \item \textbf{הרחבה לנחיל}: הארכיטקטורה מתוכננת ל-\textenglish{swarm}: חבילת \texttt{\textenglish{swarm\_coordination}} קיימת כבר ב-\textenglish{workspace}; שיתוף מפות \textenglish{log-odds} בין רחפנים וחלוקת אשכולות \textenglish{FIS} מהווים שלב הפיתוח הבא.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{גבולות תלת-מימדיים}: השימוש במפת הוקסל התלת-מימדית (כבר מוכן ב-\textenglish{voxel\_mapper}) לגילוי גבולות 3D — כמו \textenglish{FUEL} המקורי.
...
\begin{itemize}
    \item \textbf{נספח א'} — המצגת שהוצגה בכיתה ב-1.7.2026 (13 שקפים; הקובץ המקורי: \texttt{\textenglish{FUEL.pptx}} בריפו).
